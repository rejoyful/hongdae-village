...
`0013_verified_progression.sql`부터 코인·소유권뿐 아니라 집과 펫의 장기 진행도 서버가 검증한다.
...
- `0018_home_layout_library.sql`의 스냅샷은 사용자가 보낸 배치 JSON을 저장하지 않고, 집주인의 현재 서버
...
- `0019_shared_village_project.sql`은 서버 전체 공동 합계와 사용자별 도감을 분리해 보존한다. 클라이언트는
...
- 물결 테라리움 구성은 `hv-home-aquarium-{userId}`에 저장하며 경제 재화가 아니다. 실제 어항 소유와
  방 배치는 기존 서버 `inventory`·`placements`가 권위 데이터를 유지한다. `0014_home_aquarium_starter.sql`은
...
2. [`0013_verified_progression.sql`](../supabase/migrations/0013_verified_progression.sql)을 적용한다.
3. [`0014_home_aquarium_starter.sql`](../supabase/migrations/0014_home_aquarium_starter.sql)을 적용한다.
4. [`0015_four_direction_furniture.sql`](../supabase/migrations/0015_four_direction_furniture.sql)을 적용한다.
5. [`0016_furniture_workshop.sql`](../supabase/migrations/0016_furniture_workshop.sql)을 적용한다.
6. [`0017_open_homes_guestbook.sql`](../supabase/migrations/0017_open_homes_guestbook.sql)을 적용한다.
7. [`0018_home_layout_library.sql`](../supabase/migrations/0018_home_layout_library.sql)을 적용한다.
8. [`0019_shared_village_project.sql`](../supabase/migrations/0019_shared_village_project.sql)을 적용한다.
9. 인증 사용자로 다음 항목을 점검한다.
